...
type TargetMapToConflationMapLookup = Record<
  TargetMapId,
  { forward: ConflationMapId[]; backward: ConflationMapId[] }
>;
...
export type ConflationAnalysisConfig = Pick<
...
export class ConflationAnalysis {
  ...
  constructor({
    ...
    this.targetMapIdConverter =
      this.targetMap === TargetMap.NPMRDS
        ? (id: string) => id
        : (id: string) => id;

    this.targetMapIds = Object.keys(this.conflationMetrics);
    ...
    for (const targetMapId of this.targetMapIds) {
      ...
      if (!this.targetMapIsUnidirectional) {
        const { backwardConflationSegmentsLengthSum } = this.conflationMetrics[
          targetMapId
        ];
    ...
    this.forwardMatchedConflationMapIds = _.flatten(
      this.forwardMatchedTargetMapIds
        .map(
          (targetMapId) =>
            this.targetMapToConflationMapLookup[targetMapId].forward,
        )
      ...
      this.backwardMatchedTargetMapIds !== null
        ? _.flatten(
            this.backwardMatchedTargetMapIds.map(
              (targetMapId) =>
                this.targetMapToConflationMapLookup[targetMapId].backward,
            ),
          ).filter((id) => !_.isNil(id))
        : [];
  ...
  get matchingStats() {
    ...
    const matchLenDiffs = _(this.directionalMatchedTargetMapIds)
      .map((targetMapId) => [
        this.forwardMatchLengthDiffs[targetMapId],
        this.backwardMatchLengthDiffs?.[targetMapId],
      ])
    ...
    this.cachedMatchingStats.matchedR =
      ...
      (this.directionalMatchedTargetMapIds.length +
        this.directionalUnmatchedTargetMapIds.length);

    this.cachedMatchingStats.matchedLt5mR =
      this.getLengthDifferenceFilteredMatchedTargetMapIds(0, 0.005).length /
      this.numDirectionalMatchedEdges;

    this.cachedMatchingStats.matchedGte5mLt10mR =
      this.getLengthDifferenceFilteredMatchedTargetMapIds(0.005, 0.01).length /
      this.numDirectionalMatchedEdges;

    this.cachedMatchingStats.matchedGte10mLt25mR =
      this.getLengthDifferenceFilteredMatchedTargetMapIds(0.01, 0.025).length /
      this.numDirectionalMatchedEdges;

    this.cachedMatchingStats.matchedGte25mLt50mR =
      this.getLengthDifferenceFilteredMatchedTargetMapIds(0.025, 0.05).length /
      this.numDirectionalMatchedEdges;
    ...
    const fwdMatchLenDiffs = _(this.forwardMatchedTargetMapIds)
      .map((targetMapId) => [this.forwardMatchLengthDiffs[targetMapId]])
      .flatten()
      .filter((n) => Number.isFinite(n))
      .value();

    // @ts-ignore
    this.cachedMatchingStats.fwdMatchedMeanLenDiffKm = ss.mean(
      fwdMatchLenDiffs,
    );
    // @ts-ignore
    this.cachedMatchingStats.fwdMatchedMedianLenDiffKm = ss.median(
      fwdMatchLenDiffs,
    );
    ...
    this.cachedMatchingStats.fwdMatchedR =
      ...
      (this.forwardMatchedTargetMapIds.length +
        this.forwardUnmatchedTargetMapIds.length);

    this.cachedMatchingStats.fwdMatchedLt5mR =
      this.getLengthDifferenceFilteredForwardMatchedTargetMapIds(0, 0.005)
        .length / this.numForwardMatchedEdges;

    this.cachedMatchingStats.fwdMatchedGte5mLt10mR =
      this.getLengthDifferenceFilteredForwardMatchedTargetMapIds(0.005, 0.01)
        .length / this.numForwardMatchedEdges;

    this.cachedMatchingStats.fwdMatchedGte10mLt25mR =
      this.getLengthDifferenceFilteredForwardMatchedTargetMapIds(0.01, 0.025)
        .length / this.numForwardMatchedEdges;

    this.cachedMatchingStats.fwdMatchedGte25mLt50mR =
      this.getLengthDifferenceFilteredForwardMatchedTargetMapIds(0.025, 0.05)
        .length / this.numForwardMatchedEdges;

    this.cachedMatchingStats.fwdMatchedGte50mLt100mR =
      this.getLengthDifferenceFilteredForwardMatchedTargetMapIds(0.05, 0.1)
        .length / this.numForwardMatchedEdges;
    ...
    if (this.backwardMatchedTargetMapIds) {
      const bwdMatchLenDiffs = _(this.backwardMatchedTargetMapIds)
        .map((targetMapId) => [
          // @ts-ignore
          this.backwardMatchLengthDiffs[targetMapId],
        ])
      ...
      // @ts-ignore
      this.cachedMatchingStats.bwdMatchedMeanLenDiffKm = ss.mean(
        bwdMatchLenDiffs,
      );
      // @ts-ignore
      this.cachedMatchingStats.bwdMatchedMedianLenDiffKm = ss.median(
        bwdMatchLenDiffs,
      );
      ...
      this.cachedMatchingStats.bwdMatchedR =
        ...
        // @ts-ignore
        (this.backwardMatchedTargetMapIds.length +
          this.backwardUnmatchedTargetMapIds.length);

      this.cachedMatchingStats.bwdMatchedLt5mR =
        this.getLengthDifferenceFilteredBackwardMatchedTargetMapIds(0, 0.005)
          .length / this.numBackwardMatchedEdges;

      this.cachedMatchingStats.bwdMatchedGte5mLt10mR =
        this.getLengthDifferenceFilteredBackwardMatchedTargetMapIds(0.005, 0.01)
          .length / this.numBackwardMatchedEdges;

      this.cachedMatchingStats.bwdMatchedGte10mLt25mR =
        this.getLengthDifferenceFilteredBackwardMatchedTargetMapIds(0.01, 0.025)
          .length / this.numBackwardMatchedEdges;

      this.cachedMatchingStats.bwdMatchedGte25mLt50mR =
        this.getLengthDifferenceFilteredBackwardMatchedTargetMapIds(0.025, 0.05)
          .length / this.numBackwardMatchedEdges;

      this.cachedMatchingStats.bwdMatchedGte50mLt100mR =
        this.getLengthDifferenceFilteredBackwardMatchedTargetMapIds(0.05, 0.1)
          .length / this.numBackwardMatchedEdges;

      this.cachedMatchingStats.bwdMatchedGte100mR =
        this.getLengthDifferenceFilteredBackwardMatchedTargetMapIds(0.1)
          .length / this.numBackwardMatchedEdges;
    }
  ...
  private filterTargetMapIdsByMatchLengthDiffs(
    ...
    // let n = 0
    return targetMapIds.filter((targetMapId) => {
      // Assigned length diffs are NULL IFF edge is Unidirectional and other direction is matched.
      // If edge is unmatched, the length diff is the targetMapEdgeLength.
  ...
  getBackwardMatchLengthDiffFilteredSegmentIds(
    filterFn: (
      targetMapEdgeLength: number,
      backwardConflationSegmentsLengthSum: number,
    ) => boolean,
  ) {
    return Object.keys(this.conflationMetrics)
      .map((targetMapId) => +targetMapId)
  ...
  getForwardMatchedConflationMapIds(targetMapIds: TargetMapId | TargetMapId[]) {
    const tgtMapIds = Array.isArray(targetMapIds)
      ? targetMapIds
      : [targetMapIds];
  ...
  getBackwardMatchedConflationMapIds(
    targetMapIds: TargetMapId | TargetMapId[],
  ) {
    const tgtMapIds = Array.isArray(targetMapIds)
      ? targetMapIds
      : [targetMapIds];
...
export default {
  async createConflationAnalysis(targetMap: TargetMap) {
    ...
    const [conflationMappings, conflationMetrics]: [
      ConflationMappings,
      ConflationMetrics,
    ] = await Promise.all([
    ...
    console.log({ conflationMappings, conflationMetrics });

    return new ConflationAnalysis({
      targetMap,
      conflationMappings,
      conflationMetrics,
    });
