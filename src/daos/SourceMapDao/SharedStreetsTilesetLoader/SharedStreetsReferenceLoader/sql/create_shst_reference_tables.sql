...
DROP TABLE IF EXISTS shst.shst_references ;

CREATE TABLE shst.shst_references (
  id            TEXT PRIMARY KEY,
  geometry_id   TEXT,
  form_of_way   TEXT
) WITHOUT ROWID ;

CREATE INDEX shst.shst_references_geom_idx
  ON shst_references (geometry_id) ;

DROP TABLE IF EXISTS shst.shst_references_geopoly_idx;

CREATE VIRTUAL TABLE shst.shst_references_geopoly_idx
  USING geopoly(id) ;
...
DROP TABLE IF EXISTS shst.shst_references_location_references ;

-- FOREIGN KEY (intersection_id) REFERENCES shst_intersections(id)
CREATE TABLE shst.shst_references_location_references (
...
CREATE INDEX shst.shst_location_references_shst_ref_id_idx
  ON shst_references_location_references (shst_reference_id) ;

CREATE INDEX shst.shst_location_references_intxn_id_idx
...
DROP TABLE IF EXISTS shst.shst_reference_forms_of_way ;

CREATE TABLE shst.shst_reference_forms_of_way
  AS
    SELECT
        column1 AS element,
        column2 as value
...
DROP TABLE IF EXISTS shst.shst_references_location_references_geopoly_idx;

CREATE VIRTUAL TABLE shst.shst_references_location_references_geopoly_idx
